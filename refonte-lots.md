...
| ~~*à lettrer*~~ **= lot « Moi »**, ✅ 2026-09-17 *(partiel, et le retrait est une décision)* | ~~Moi, rangs 2 et 3 : part du commun et dépenses solo en cartes~~, **liste solo**, ~~enveloppes solo~~. **LA CARTE « TA PART DU COMMUN » EST ABANDONNÉE, sur mesure** : `suiteDeMoi` (`summary.js`) affiche déjà la ligne, textuellement, sous le héros, et son grand-livre n'est PAS un `<details>` — son ouverture permanente est une décision prise avec son prix chiffré (300 px à 320) et sa raison. « La carte ouvre le grand-livre » n'avait donc pas d'objet. **La colonne des cartes reste vide sous « Moi » et « Privé », et ce n'est plus une question ouverte** — la réponse du foyer est écrite dans `summary.css`, « La portée gouverne le panneau ». Ce que le lot livre : **le filtre sur « Moi »** (→ MON personnel seul ; `chargesDeLaPortee` prend `moi`, obligatoire — sans lui, sous un aval, l'écran montrerait le personnel de l'autre), **une SECONDE NATURE de renvoi** (le commun n'est pas dans cette liste, il est dans « À deux » ; **zéro chiffre** — le montant est dans le grand-livre, et ce qui manquait était une adresse), et **l'état vide par une fabrique qui construit le DOM** (une chaîne assignée à `innerHTML` porterait le plafond de 24 à 26). Les enveloppes solo restent à faire | — |
...
| **P1a** | **Le mur, le schéma, la sauvegarde et les contrôles — AUCUNE donnée déplacée.** ✅ 2026-09-16. `household/personnel/{qui}` reçoit le `.read` de `prive/` repris verbatim (propriétaire, ou l'autre sous `aval/{qui}/actif`) et le schéma COMPLET d'une charge ; le `.read` quitte la racine des deux espaces pour leurs 13 enfants directs, parce qu'un `.read` cascade et ne se révoque pas plus bas ; la sauvegarde lit nœud par nœud et la restauration n'écrase plus la racine d'un `set`. **`personnel/{qui}` reste VIDE** : déplacer les 3 charges avant que les 37 points de passage sachent lire les deux poches les ferait cesser de compter le personnel **en restant verts** | — |
| **P1b** | **La lecture rétablie, les écritures dérivées, les 3 charges migrées.** ✅ 2026-09-16, en trois commits. *(1)* une seule fabrique de fusion, `poches.js`, lue par les 19 sites de lecture : `getState('variableCharges')` et `getState('fixedCharges')` portent de nouveau le commun ET le personnel qu'on a le droit de lire — les 33 `getState` ont suivi gratuitement, comme le relevé l'annonçait. Le miroir est ASYMÉTRIQUE et assumé : sa poche est mémorisée et mise en file hors ligne, celle de l'autre jamais. *(2)* **le chemin se dérive de la charge, il ne se compose plus** — `cheminDeLaCharge`, aux 19 sites d'écriture ; la bascule « perso » est nommée, confirmée et atomique ; la migration des 3 charges se fait à l'ouverture, **chacun la sienne**, parce que personne n'écrit dans la poche de l'autre. Un défaut réel trouvé au passage : `quick-add.js` écrivait « Perso » dans le COMMUN. *(3)* reconduction et corbeille par poche, avec **une marque de reconduction par poche** — le seul nœud que ce lot ajoute aux règles. | — |
| ~~**P1b** *(cadrage d'origine)*~~ | ~~**Les 37 points de passage, puis la migration EN DERNIER.** Chacun avec un verdict écrit — compte, ou ne compte pas : 3 lecteurs de liste (`variable-charges:330`, `fixed-charges:349`, `quick-add:813`), 16 lecteurs d'historique (`dbGet('periods')`, dont 5 acceptent une injection), 18 traversées à nom de collection en dur. Les 33 `getState` suivent gratuitement si la fusion est faite dans les 2 chargeurs. La migration des 3 charges `perimetre: 'solo'` (1 mois, `2026-09`, 0 sans propriétaire — relevé sur sauvegarde le 2026-09-16) vient quand tout sait les lire~~ | — |
| ~~**P1**~~ | ~~**Le personnel sort de `household/`.**~~ **Scindé en P1a / P1b le 2026-09-16**, sur le relevé qui a chiffré les points de passage. Texte d'origine conservé : Décision de Richie, 2026-09-16 (`CLAUDE.md`, *Décisions de conception*). Les dépenses solo passent sous le mécanisme d'aval de `prive/` : lisibles par leur propriétaire, par l'autre seulement si `aval/{qui}/actif`. Migration des données existantes comprise. **Cas neufs dans `tests/regles/mur-prive.test.js`**, qui interroge l'émulateur et non les règles | — |
| **P2** | **Le périmètre entre dans la liste — filtrage, total de catégorie et renvoi, un seul lot.** ✅ 2026-09-17. « À deux » → **commun seul**, par `chargesDeLaPortee` (`utils/portee.js`), qui lit `perimetre.js` et **jamais le payeur** : une charge avancée par une personne et partagée est COMMUNE, et un filtre par payeur la perdrait en silence pendant que le solde continuerait de la compter. Le jeu d'essai porte les **quatre combinaisons** — payeur × périmètre — parce que trois d'entre elles ne séparent pas les deux prédicats. **Le total de catégorie annonce le personnel**, `commun + X perso` et seulement s'il existe : la distinction entre dans `grouperParCategorie` (`utils/tri.js`), qui rend désormais le couple, et `totaux-liste.js` cesse de la recalculer par-dessus — **une seule rédaction**, `libelleDuTotal`, lue par le pied ET par l'en-tête. **Le renvoi en pied est CRÉÉ** (il n'existait pas) : il nomme sa destination, ne chiffre que MES dépenses, et corrige la seule affirmation que filtrer rend fausse — un mois qui ne porte que du personnel ne s'annonce plus vide. C'est un **bouton NU** : un attribut d'état ARIA en ferait une seconde annonce de portée pour `portee-unique.spec.js`. **Décisions bornées** : « Moi ce mois » n'est PAS filtré ~~(il le sera au lot « Moi, rangs 2 et 3 »)~~ — **c'est fait le 2026-09-17, sans la carte** ; ni la recherche historique ni la corbeille ne le sont ; l'en-tête est **borné** aux catégories qui gardent une ligne affichée | — |
...
> **⟲ LA COMMANDE DE P1 A CHANGÉ DE FICHIER, LE 2026-09-16, DANS LE COMMIT DU
> LOT P1a.** Elle cherchait `cheminDuPersonnel` dans `public/js/utils/perimetre.js` ;
> elle le cherche dans `public/js/db.js`.
>
> **La raison.** `perimetre.js` se déclare en tête « ne contient que des
> fonctions pures » et ne sait rien de l'arbre de la base — ni `personnel`, ni
> `periods`, ni le préfixage d'espace que `?sandbox=1` bascule. Y poser une
> fabrique de chemin lui aurait fait connaître les trois, et aurait donné à un
> module de 18 dépendants une raison de changer chaque fois que l'arbre bouge.
> La connaissance de l'arbre vit dans `db.js`, avec `getDataPath` et les quatre
> accès absolus.
>
> Le nom de la fonction, lui, est inchangé : c'est le fichier qui était mal
> choisi, pas la commande.

| Lot | Commande | Attendu |
|---|---|---:|
| **P1a** | `@(Select-String -Pattern "export function cheminDuPersonnel" -Path public/js/db.js).Count` | ≥ 1 |
| **P1a** | `@(Select-String -Pattern "Le mur personnel" -Path tests/regles/mur-prive.test.js).Count` | ≥ 1 |
