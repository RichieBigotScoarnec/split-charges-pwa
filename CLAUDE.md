...
| Après une lecture ratée, les listes personnalisées repartaient des défauts — et `fusionnerListe` prenait ce repli pour « ce que la session connaît » : ajouter une catégorie réécrivait « Courses » par-dessus « Supermarché », renommage perdu et charges détachées | `public/js/modules/custom-lists.js` | ✅ RÉSOLU 2026-08-26 — écriture refusée tant que la lecture n'a pas abouti | Les enveloppes replient sur la liste vide, qui ne prétend rien connaître |
| `_isLoading` était relevé puis rabaissé à la main : une levée de `calculateSummary` le laissait haut pour la session, et plus aucun changement de mode n'était enregistré | `public/js/modules/share-mode.js` | ✅ RÉSOLU 2026-08-26 — `finally`, par une seule voie | Rendu atteignable par la remontée d'erreur du même lot |
| **Une charge saisie en septembre était rangée en juillet, et le total de juillet la comptait.** Le formulaire pré-remplit la date du JOUR ; l'écriture visait le mois AFFICHÉ. Rien ne comparait les deux : consulter un mois passé suffisait à y ranger une dépense d'aujourd'hui, datée d'aujourd'hui | `public/js/modules/variable-charges.js`, `quick-add.js`, `public/js/utils/date.js` | ✅ RÉSOLU 2026-09-01 — `periodeDeLaDate` : la date fait foi, et l'écran nomme le mois d'arrivée quand ce n'est pas celui qu'on regarde. Témoin négatif joué : sans le correctif, le contrôle du cas réel reçoit `periods/2026-07` | Ce n'était pas un défaut de rangement : le solde entre les deux personnes était faux du montant. L'édition ne déplace toujours pas une charge — deux écritures non atomiques la dupliqueraient. `tools/charges-mal-rangees.mjs` relève l'existant sur une sauvegarde, sans rien modifier |
...
### Quatre finitions, dont une qui débloquait une couverture — 2026-09-05

Quatre points sans lien, tous petits, tous consignés. Le troisième s'est révélé
n'être pas un renommage : c'est ce qui empêchait un témoin écrit une heure plus
tôt de couvrir la moitié de ce qu'il regarde.

| Déclaration CLAUDE.md | Fichier réel | État | Action |
|---|---|---|---|
| **La modale du détail disait la PARTIALITÉ, jamais la RÈGLE.** Elle est la quatrième surface dont `splitOverride` change le chiffre, et la dernière à ne rien en dire : « part de 1 000,00 € » annonce que la ligne ne compte pas en entier, sans nommer ce qui a produit ce montant. C'est pourtant là que la question se pose — on vient d'ouvrir un total pour comprendre d'où il sort, et une charge en 50/50 dans un foyer au prorata y entre pour 500,00 € quand la règle commune donnerait 750,00 | `public/js/modules/detail-depenses.js` | ✅ RÉSOLU 2026-09-05 — `libelleDeLaRepartition`, la fabrique du matin ; `detail.js:92` poussait déjà la charge entière dans la ligne, rien n'est relu ni recalculé | Les deux mentions se complètent, elles ne se remplacent pas — un contrôle l'exige. Une cinquième rédaction de la grammaire aurait été la neuvième occurrence du défaut `normalizePair` |
| Le prédicat est celui des trois autres surfaces — « la charge porte un `splitOverride` » — **y compris sur un détail de CATÉGORIE**, où le montant affiché est plein | `public/js/modules/detail-depenses.js` | ⚠️ DÉCIDÉ 2026-09-05 — le faire dépendre de `surLaPart` donnerait deux réponses pour la même charge selon le chiffre par lequel on l'a ouverte | Même arbitrage que le 2026-09-05 au matin sur le récap, pour la même raison |
| **Aucun contrôle de cohérence visuelle ne visite cette modale** : `coherence-visuelle.spec.js` ne connaît que les trois panneaux, `theme-enveloppe.spec.js` que la modale des enveloppes. Ajouter une étiquette dans une cellule de grille sans `min-width: 0` était donc une supposition | `tests/e2e/detail-depenses.spec.js` | ✅ RÉSOLU 2026-09-05 — trois propriétés à 390 px et à 320 px : `scrollWidth === clientWidth`, rien hors du cadre, la pastille ne recouvre pas le montant. Mesuré sur un libellé de 40 caractères : **0 px de hauteur en plus à 390 px, +22 px à 320 px**, et 41 px entre la pastille et le montant | Le contrôle sait tomber : `white-space: nowrap` sur le titre le fait passer au rouge aux deux largeurs, avec les quatre débordements nommés. Et un témoin exige la pastille, sans quoi les trois mesures seraient vraies d'une modale qui n'en porte aucune |
| **`charge-split-tag` portait DEUX sémantiques** : la répartition dérogatoire — quatre surfaces — et le mot « fixe » (`detail-depenses.js`, `envelopes.js`). Chercher où `splitOverride` se voit rendait quatre sites dont deux étrangers | `public/js/modules/detail-depenses.js`, `envelopes.js`, `public/css/summary.css` | ✅ RÉSOLU 2026-09-05 — le second sens passe sous `charge-nature-tag` ; le premier ne bouge pas, il est majoritaire et tenu par des témoins | Le second site a son mutant : remettre l'ancienne classe dans la vue d'enveloppe fait tomber le contrôle E2E, qui ne trouve plus l'élément. Le TEXTE y était déjà tenu (`'Location gîte fixe'`) ; c'est la classe que rien ne regardait |
| **Et ce n'était pas un renommage, c'était une couverture bloquée.** Le témoin de la pastille avait dû se borner aux charges VARIABLES : une charge fixe dérogatoire portait deux pastilles de même classe, et le comptage mesurait la collision. La moitié fixe de `chargesRetenues` restait sans témoin dans cette modale | `tests/modules/badge-repartition.test.js` | ✅ RÉSOLU 2026-09-05 — le loyer entre au jeu d'essai, en charge fixe ET en 50/50 ; un contrôle exige que la ligne porte les deux étiquettes sous leurs deux classes | **Le rouge EST la collision**, et il s'est manifesté seul : « expected 'fixe' to be '50/50' » — la première pastille de la ligne était l'étiquette de nature — plus un comptage de 3 là où 2 répartitions sont affichées. Aucun cas fabriqué |
| Les deux sens ne partageaient pas des règles de style : ils partageaient **LA** règle — un bloc unique de dix déclarations, sans divergence | `public/css/summary.css` | ✅ RÉSOLU 2026-09-05 — une seule liste de sélecteurs pour les deux noms : le rendu est identique par construction, et le reste | Dupliquer ces dix lignes aurait fabriqué la copie que ce journal raconte huit fois. Une copie ne se dégrade pas d'un coup, elle se dégrade au correctif suivant que personne n'y reporte |
| « fixe » est une NATURE, comme « ponctuelle » — qui, elle, est peinte en ambre (`.charge-ponctuel`). Le neutre avait été justifié pour la répartition, jamais pour la nature | `public/css/summary.css` | ⚠️ OUVERT, NON TRANCHÉ — la question est écrite dans le CSS plutôt que résolue par omission ; ce lot ne change rien à ce qui est peint | Séparer les classes rend le choix POSSIBLE ; le faire en passant aurait été le prendre sans le poser |
| Les cinquante-sept propositions du sélecteur d'emoji nommaient toutes un DOMAINE — une maison, une voiture, un café. Une charge fixe ne se définit pas par un domaine mais par son RYTHME : elle revient. Un abonnement, un prélèvement, une cotisation ne sont d'aucune famille, et le foyer n'avait que 🏠 pour se les représenter — le seul glyphe qui pouvait à peu près faire l'affaire, et qui dit autre chose | `public/js/modules/custom-lists.js` | ✅ RÉSOLU 2026-09-05 — 🔁, rangé en « Comptes et virements » : ce qui revient tous les mois relève de la mécanique du compte, aux côtés de 🏦, 💰 et 🧾, pas des objets du quotidien que sont 👕 et 💇 | Le contrôle des propositions borne leur nombre par un INTERVALLE (40–72), pas par 57 : il n'y avait aucun chiffre à corriger, mais une propriété à ajouter — couvrir le rythme, pas seulement le domaine |
| **La présentation du glyphe a été mesurée des deux côtés avant de l'inscrire.** `encre-rendue.spec.js` écarte les emoji de présentation couleur et mesure au seuil de 3:1 les glyphes qui suivent `color` : un glyphe du mauvais côté serait entré dans une garde qui ne l'attend pas | `public/js/modules/custom-lists.js` | ✅ VÉRIFIÉ 2026-09-05 — U+1F501 porte `Emoji_Presentation=Yes`, donc le motif d'exclusion l'écarte ; et Chromium le peint bien par la police — rendu sur canevas sous deux encres, pixels identiques, comme 🏠 et ☕, quand ✕ et ◀ suivent `color` | La propriété Unicode et le rendu réel disent la même chose. L'un sans l'autre n'aurait prouvé que la moitié |
