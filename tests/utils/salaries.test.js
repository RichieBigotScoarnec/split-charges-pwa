...
/*
   La forme normalisée porte désormais quatre champs : les deux salaires et
   les deux revenus complémentaires. Ces derniers sont absents des entrées de
   ces tests, ils valent donc zéro — ce qui vérifie au passage que les données
   antérieures à la fonctionnalité restent lues correctement.
*/
...
describe('resolveSalaries', () => {
  ...
  it("privilégie l'instantané de la période quand il existe", () => {
    ...
    expect(result.salaries).toEqual({ vous: 2500, conjointe: 1800, extraVous: 0, extraConjointe: 0 });
  ...
  it('retombe sur les salaires globaux sans instantané', () => {
    ...
    expect(result.salaries).toEqual({ vous: 3000, conjointe: 2000, extraVous: 0, extraConjointe: 0 });
  ...
  it("n'écrase pas un instantané à zéro par les globaux", () => {
    ...
    expect(result.salaries).toEqual({ vous: 0, conjointe: 0, extraVous: 0, extraConjointe: 0 });
    expect(result.fromSnapshot).toBe(true);
  });
  ...
  it("isole les périodes : deux instantanés distincts donnent deux résultats", () => {
    ...
    expect(mars.salaries).toEqual({ vous: 2000, conjointe: 1500, extraVous: 0, extraConjointe: 0 });
    expect(aout.salaries).toEqual({ vous: 3200, conjointe: 2100, extraVous: 0, extraConjointe: 0 });
  });

  it('normalise aussi les instantanés mal typés', () => {
    const result = resolveSalaries({ vous: '2500', conjointe: -5 }, global);

    expect(result.salaries).toEqual({ vous: 2500, conjointe: 0, extraVous: 0, extraConjointe: 0 });
    expect(result.fromSnapshot).toBe(true);
  });
});
