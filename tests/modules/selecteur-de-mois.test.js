...
describe('Douze mois consécutifs, quel que soit le jour', () => {
  ...
  for (const jour of [29, 30, 31, 15]) {
    it(`le ${jour} mars, la liste ne saute ni ne double aucun mois`, () => {
      ...
      expect(new Set(mois).size, `doublon dans ${mois.join(', ')}`).toBe(12);
      expect(mois[0]).toBe('2026-03');
      expect(mois[1], 'février doit suivre mars').toBe('2026-02');
    });
  }

  it('le 31 mai, la suite reste continue sur douze mois', () => {
