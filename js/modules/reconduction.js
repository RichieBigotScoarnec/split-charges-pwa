...
export async function proposeReconduction(targetPeriod = null) {
  ...
  try {
    const snapshot = await database.ref(`periods/${targetPeriod}`).once('value');
...
export async function executeReconduction(sourcePeriod, targetPeriod, options = {}) {
  ...
  try {
    ...
    if (fixedCharges) {
      const fixedSnapshot = await database.ref(`periods/${sourcePeriod}/fixedCharges`).once('value');
      if (fixedSnapshot.exists()) {
        ...
        const activeCharges = Object.entries(charges)
          ...
          .reduce((acc, [_, charge]) => {
            ...
            acc[`periods/${targetPeriod}/fixedCharges/${newKey}`] = {
              ...charge,
              timestamp: Date.now()
            };
            return acc;
    ...
    // Copier les salaires
    if (salaries) {
      const salariesSnapshot = await database.ref(`periods/${sourcePeriod}/salaries`).once('value');
      if (salariesSnapshot.exists()) {
        updates[`periods/${targetPeriod}/salaries`] = salariesSnapshot.val();
      }
    }

    // Copier le mode de partage
    const shareModeSnapshot = await database.ref(`periods/${sourcePeriod}/shareMode`).once('value');
    if (shareModeSnapshot.exists()) {
      updates[`periods/${targetPeriod}/shareMode`] = shareModeSnapshot.val();
    }
...
export async function periodExists(period) {
  try {
    const snapshot = await database.ref(`periods/${period}`).once('value');
