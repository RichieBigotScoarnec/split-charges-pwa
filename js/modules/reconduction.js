...
export async function proposeReconduction(targetPeriod = null) {
  ...
  try {
    const snapshot = await database.ref(getUserPath(`periods/${targetPeriod}`)).once('value');
...
export async function executeReconduction(sourcePeriod, targetPeriod, options = {}) {
  ...
  try {
    ...
    if (fixedCharges) {
      const fixedSnapshot = await database.ref(getUserPath(`periods/${sourcePeriod}/fixedCharges`)).once('value');
      if (fixedSnapshot.exists()) {
        ...
        const activeCharges = Object.entries(charges)
          ...
          .reduce((acc, [_, charge]) => {
            ...
            acc[getUserPath(`periods/${targetPeriod}/fixedCharges/${newKey}`)] = {
              ...charge,
              timestamp: Date.now()
            };
            return acc;
    ...
    // Copier les salaires
    if (salaries) {
      const salariesSnapshot = await database.ref(getUserPath(`periods/${sourcePeriod}/salaries`)).once('value');
      if (salariesSnapshot.exists()) {
        updates[getUserPath(`periods/${targetPeriod}/salaries`)] = salariesSnapshot.val();
      }
    }

    // Copier le mode de partage
    const shareModeSnapshot = await database.ref(getUserPath(`periods/${sourcePeriod}/shareMode`)).once('value');
    if (shareModeSnapshot.exists()) {
      updates[getUserPath(`periods/${targetPeriod}/shareMode`)] = shareModeSnapshot.val();
    }
